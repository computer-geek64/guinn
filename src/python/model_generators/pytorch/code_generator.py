    ...
    for i in range(len(layers)):
        if (layers[i]['name'] == 'dense'):
            code_template += 'layers_list.append(nn.Linear({previous_layer_nodes}, {layer_nodes}))'\
                .format(previous_layer_nodes=layers[i-1]['nodes'], layer_nodes=layers[i]['nodes'])
            ...
            code_template += 'layers_list.append(nn.Dropout2d(p={probability}))'\
                .format(probability=layers[i]['rate'])
        elif (layers[i]['name'] == 'batch_normalization'):
            code_template += 'layers_list.append(nn.BatchNorm1d(num_features={num_of_features}, eps={eps}, momentum={momentum}))'\
                .format(num_of_features=layers[i]['num_of_features'], eps=layers['i']['epsilon'], momentum=layers['i']['momentum'])
        elif(layers[i]['name'] == 'conv_2d'):
            code_template += 'layers_list.append(nn.Conv2d({input_channels}, {output_channels}, {kernel_size}, stride={stride}))'\
            ...
            code_template += 'layers_list.append(nn.MaxPool2d(kernel_size = {kernel_size}, stride={stride}))'\
                .format(kernel_size = layers[i]['kernel_size'], stride=layers[i]['stride'])
